...
class Concatenator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        out = [self.glue.join(x) if cols > 1 else x for x in X]
        return out
...
class WordCleaner(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        def _text_clean(x):
            all_clean = BeautifulSoup(self.decode(x), "lxml").get_text().lower()
            replaced = reduce(lambda acc, re_rep: re.sub(re_rep[0], re_rep[1], acc), self.feature_regex_pipe, all_clean)
...
def get_transformer(name):
    transformer_map = {
        ...
        "wordcount"        : build_wordcount_transformer,
